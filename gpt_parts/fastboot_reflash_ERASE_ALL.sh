...
for y in aboot sbl1 boot; do
    fastboot erase ${y}
    done
for x in modem sbl1 sbl1bak aboot abootbak rpm rpmbak tz tzbak hyp hypbak pad modemst1 modemst2 misc fsc ssd splash DDR fsg sec boot system persist cache recovery userdata; do
    fastboot flash ${x} ${x}.bin
done

## if GPT partitions need to be flashed, uncomment next two lines
## fastboot flash PrimaryGPT gpt_main0.img
## fastboot flash BackupGPT gpt_backup0.img
